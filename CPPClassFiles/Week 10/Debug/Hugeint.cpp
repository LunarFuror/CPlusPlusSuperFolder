// Exercise 10.9 Solution: Hugeint.cpp
...
size_t HugeInt::getLength() const
{
   size_t i;
...
HugeInt HugeInt::operator+( int op2 ) const //added operator
{
...
{
   for ( size_t i = 0; i < digits; ++i ) // compare each element
...
HugeInt HugeInt::operator-( const HugeInt &op2 ) const
{
   ...
   if ( op2 > *this )
      throw invalid_argument( 
         "Tried to subtract larger value from smaller value." );
...
HugeInt HugeInt::operator*( const HugeInt &op2 ) const
{
   ...
   for ( size_t i = digits; i > digits - smaller.getLength(); --i )
   {
      ...
      for ( size_t j = digits; j > digits - larger.getLength(); --j )
      {
...
HugeInt HugeInt::operator/( const HugeInt &op2 ) const
{
   ...
   for ( size_t i = digits - maxSolutionLength; i < digits; ++i )
   {
